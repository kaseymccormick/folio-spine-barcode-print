...
- `wrangler.toml` (repo root) is committed and is the source of truth for the deploy target: worker name `folio-spine-barcode-print`, assets served from `./dist`. Confirmed working post-push — the live site still serves from the same origin with the expected `public/_headers` CSP intact (checked via `curl -I`), so it updated the existing Worker rather than creating a new one.
...
- `FolioSettings.tsx`'s credentials disclaimer (`text-warning` in light mode, orange) uses `dark:text-white dark:font-bold` instead of `text-warning` in dark mode — the orange read as jarring against the all-blue dark palette. This is a one-off `dark:` override, not a token change; if `--warning` or the dark palette changes again, re-check this line specifically.
