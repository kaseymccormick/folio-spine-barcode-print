...
- `--ring` (focus outline color) matches `--accent` (`#097510`) in both themes, by design — not part of the brand blue family, kept as a deliberate green interactive accent.

## Dark mode — toggle is live, theme rebuilt from brand palette
...
- `theme.css` `.dark` neutral scale is a deliberate elevation hierarchy, darkest to lightest: `--background: #3f4444` → `--card: #53565a` → `--secondary: #97999b` → `--muted: #c8c9c7`, each with a contrast-checked foreground. `--border`/`--input` use `#406098` (brand blue-gray) instead of a generic gray. `--primary` (`#0033a0`) and `--warning` (`#d64309`) now match light mode exactly instead of using invented/generic values — `--accent`/`--ring` stay `#097510` in both themes too. The only thing that changes between themes is the neutral scale; brand colors stay constant.
