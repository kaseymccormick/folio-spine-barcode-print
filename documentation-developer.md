...
- **Dark mode toggle is live** (header button, top right). Contrast bugs on `--card-foreground`, `--muted-foreground`, `--input-background`, and `--switch-background` are fixed — see `CLAUDE.md` → "Dark mode" for the one known remaining tradeoff (`RequestLog`'s collapsed summary bar).
