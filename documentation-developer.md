...
- **`wrangler.toml`** drives deploy config now (previously the Cloudflare dashboard only). Confirmed working: post-push, the live site still serves from the same Worker/origin with `public/_headers` CSP intact — it updated the existing Worker rather than creating a new one.
...
- **Dark mode toggle is live** (header button, top right), theme rebuilt from the Boise State brand palette — see `CLAUDE.md` → "Dark mode" for the color hierarchy and the `text-muted-foreground`/`text-secondary-foreground` fix behind it.
