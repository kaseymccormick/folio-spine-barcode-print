...
- **`npm audit` isn't automated.** Most dependencies are pinned to exact versions (no `^`) in `package.json`, so security patches require a manual bump + `npm install`, not `npm update`.
