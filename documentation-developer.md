...
- **Dark mode toggle is live** (header button, top right), but `--input-background` and `--switch-background` have no `.dark` override — see `CLAUDE.md` → "Dark mode" for what's still needed before it's fully correct.
